...
exit 1'); then
  ...
  runtime-config)
    ...
    if [[ ! -f "$env_file" ]]; then
      ...
      source "$secrets_file"
      : "${TF_VAR_rds_password:?missing protected RDS password}"
      : "${TF_VAR_redis_password:?missing protected Redis password}"
      runtime_outputs=$(mktemp); TEMP_FILES+=("$runtime_outputs"); chmod 600 "$runtime_outputs"
      terraform -chdir="$terraform_dir" output -json >"$runtime_outputs" || die "cannot read Terraform runtime outputs"
      install -d -m 700 "$(dirname -- "$env_file")"
      jq -r --arg rdsPassword "$TF_VAR_rds_password" --arg redisPassword "$TF_VAR_redis_password" '
        def line($key; $value): $key + "=" + ($value | @sh);
        [
          line("SPRING_DATASOURCE_URL"; "jdbc:mysql://" + .rds.value.connection + ":" + .rds.value.port + "/" + .rds.value.database + "?useUnicode=true&characterEncoding=utf8&serverTimezone=Asia%2FShanghai"),
          line("SPRING_DATASOURCE_USERNAME"; .rds.value.account),
          line("SPRING_DATASOURCE_PASSWORD"; $rdsPassword),
          line("REDIS_HOST"; .redis.value.connection),
          line("REDIS_PORT"; (.redis.value.port | tostring)),
          line("REDIS_PASSWORD"; $redisPassword),
          line("OSS_ENDPOINT"; "https://" + .oss.value.runtime_endpoint),
          line("OSS_PUBLIC_ENDPOINT"; "https://" + .oss.value.control_endpoint),
          line("OSS_BUCKET"; .oss.value.artifact_bucket),
          line("SLS_ENDPOINT"; "https://" + .sls.value.runtime_endpoint),
          line("SLS_PROJECT"; .sls.value.project),
          line("SLS_SYS_LOGSTORE"; .sls.value.stores.system),
          line("SLS_BIZ_LOGSTORE"; .sls.value.stores.business),
          line("SLS_METRIC_LOGSTORE"; .sls.value.stores.metrics),
          "AUTOWONDER_AONE_ENABLED=false",
          "AUTOWONDER_SLS_ENABLED=true",
          "AUTOWONDER_SIGAR_ENABLED=true"
        ] | .[]
      ' "$runtime_outputs" >"$env_file"
      chmod 600 "$env_file"
      unset TF_VAR_rds_password TF_VAR_redis_password TF_VAR_ecs_password secrets_file runtime_outputs
    fi
    require_file "$env_file"; require_mode_600 "$env_file"
...
exit 0")
  ...
  rolling-start)
    ...
    alb_id=$(jq -er '.resources.load_balancer_id // .resources.alb_id // empty' "$manifest") || die "ALB ID is required for DNS-independent rolling checks"
      ...
      for attempt in $(seq 1 18); do
        ...
        if aliyun_cli alb GetListenerHealthStatus --region "$region" --ListenerId "$listener_id" >"$health_file" \
          && jq -e --arg instance "$instance" '
            .ListenerHealthStatus | length > 0 and
            all(.[]; all(.ServerGroupInfos[]?;
              all((.NonNormalServers // [])[]?; .ServerId != $instance)))
...
printf '\\nBUSINESS_STATUS=passed\\n'"
  ...
  acceptance)
    ...
    if [[ "$operation_scope" == deployment ]]; then
      if ! jq -e '(.resources.alb_public_ipv4_addresses // .resources.albPublicIpv4Addresses) | type == "array" and length > 0' "$manifest" >/dev/null; then
        alb_id=$(jq -er '.resources.load_balancer_id // .resources.alb_id // empty' "$manifest") || die "ALB ID missing for public acceptance"
        alb_file=$(mktemp); TEMP_FILES+=("$alb_file"); chmod 600 "$alb_file"
        aliyun_cli alb GetLoadBalancerAttribute --region "$region" --LoadBalancerId "$alb_id" >"$alb_file" || die "ALB address inventory failed"
        public_ips_json=$(jq -c '[.ZoneMappings[]?.LoadBalancerAddresses[]?.Address] | map(select(type == "string" and length > 0)) | unique' "$alb_file")
        [[ $(jq 'length' <<<"$public_ips_json") == 2 ]] || die "deployment acceptance requires exactly two distinct ALB public IPv4 addresses"
        atomic_jq "$manifest" --argjson addresses "$public_ips_json" '.resources.alb_public_ipv4_addresses=$addresses'
      fi
      public_ips=()
...
printf '\''BASIC_ACCEPTANCE=passed\n'\''')
    ...
    if [[ "$protocol" == domain-with-certificate ]]; then
      alb_id=$(jq -er '.resources.load_balancer_id // .resources.alb_id // empty' "$manifest") || die "ALB ID missing for TLS acceptance"
      listeners_file=$(mktemp); TEMP_FILES+=("$listeners_file"); chmod 600 "$listeners_file"
      aliyun_cli alb ListListeners --region "$region" --MaxResults 100 >"$listeners_file" || die "ALB listener inventory failed"
