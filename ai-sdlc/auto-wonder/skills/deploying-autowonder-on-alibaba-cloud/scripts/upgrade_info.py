...
def sanitize_inventory(inventory: Dict[str, Any]) -> Dict[str, Any]:
    allowed = (
        ...
        "alb_id", "alb_address", "load_balancer_id", "load_balancer_address",
        "package_bucket", "artifact_bucket", "oss_endpoint",
        "oss_public_endpoint", "expected_tags",
    )
    result = {key: inventory[key] for key in allowed if inventory.get(key) not in (None, "", {}, [])}
    # Deployment and teardown use the module output names; existing upgrade
    # operations still consume the historical ALB aliases. Keep both identical.
    for canonical, legacy in (("load_balancer_id", "alb_id"), ("load_balancer_address", "alb_address")):
        value = unique_value("ALB identity", [result.get(canonical), result.get(legacy)], required=False)
        if value is not None:
            result[canonical] = result[legacy] = value
    nested_allowed = {
...
def normalize_terraform_outputs(outputs: Dict[str, Any], bindings: Dict[str, str]) -> Dict[str, Any]:
    ...
    for canonical, target in field_names.items():
        ...
        # Existing discovery records may still name the old outputs. Read the
        # real module names as well, rejecting conflicting identities instead
        # of silently preferring either source.
        aliases = {
            "albId": ("load_balancer_id", "alb_id"),
            "albAddress": ("load_balancer_address", "alb_dns_name", "alb_address"),
        }.get(canonical)
        if aliases:
            value = unique_value("ALB Terraform outputs", [value] + [terraform_value(outputs, name) for name in aliases], required=False)
...
def atomic_write_json(path: Path, value: Dict[str, Any]) -> None:
    ...
    try:
        ...
        os.chmod(path, 0o600)
        operations_checkpoint(path, value)
    finally:
        if temporary.exists():
            temporary.unlink()
...
def build_discovery(project_root: Path, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "outputBindings": {
            ...
            "albId": "load_balancer_id", "albAddress": "load_balancer_address", "rds": "rds",
