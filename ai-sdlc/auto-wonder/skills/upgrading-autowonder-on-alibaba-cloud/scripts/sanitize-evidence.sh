...
if jq -e . "$input" >/dev/null 2>&1; then
  jq '
    def sensitive_key:
      ...
      test("^(password|passwordhash|secret|accesskey|accesskeyid|accesskeysecret|masterkey|autowondersecretmasterkey|jwt|jwtsecret|token|authorization|presignedurl|credential|commandoutput|envsha256|environmentsha256|environmentplansha256|environmentcandidatesha256|candidatesha256|lastenvironmentsha256)$");
