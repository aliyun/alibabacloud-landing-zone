...
def main(argv=None):
    ...
    parser.add_argument('command', choices=('resolve', 'initialize', 'checkpoint', 'assert-current'))
...
def verify_legacy(manifest):
    ...
    if os.name == 'nt':
        commands = [['powershell', '-NoProfile', '-File', str(UPGRADE / 'verify-deployment-targets.ps1'), '-Manifest', str(manifest)],
                    ['powershell', '-NoProfile', '-File', str(UPGRADE / 'upgrade-operations.ps1'), 'upgrade-inventory', '-Manifest', str(manifest)]]
    else:
        commands = [['bash', str(UPGRADE / 'verify-deployment-targets.sh'), '--manifest', str(manifest)],
                    ['bash', str(UPGRADE / 'upgrade-operations.sh'), 'upgrade-inventory', '--manifest', str(manifest)]]
