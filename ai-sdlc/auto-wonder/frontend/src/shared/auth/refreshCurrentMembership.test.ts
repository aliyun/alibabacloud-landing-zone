...
describe('refreshCurrentMembership', () => {
  ...
  it('clears only the invalid organization after membership removal', async () => {
    server.use(
      http.get('/api/orgs/current/membership', () => HttpResponse.json({
        success: false,
        code: '11001',
  ...
  it('keeps the current snapshot after a transient refresh failure', async () => {
    server.use(
      http.get('/api/orgs/current/membership', () => HttpResponse.error()),
    );
  ...
  it('starts a separate refresh after the token and organization change', async () => {
    let releaseFirstRequest: (() => void) | undefined;
    const firstRequestGate = new Promise<void>((resolve) => {
      releaseFirstRequest = resolve;
    });
    server.use(
      http.get('/api/orgs/current/membership', async ({ request }) => {
    ...
    useAuthStore.getState().setCurrentOrg(
      { id: 8, name: 'Next Org', description: '' },
      'ADMIN',
    );
