...
import { Alert, Card, Empty, Space, Spin, Tabs, Typography } from 'antd';
...
export function ChannelIntegrationPage() {
  ...
  const items = CHANNEL_REGISTRY.filter((channel) => channel.key === activeKey).map((channel) => ({
    key: channel.key,
    label: channel.label,
    ...
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Card>
        <Space direction="vertical" size={4} style={{ width: '100%' }}>
          ...
          <Text type="secondary">
            把数字人接入到消息渠道,群成员 @数字人 即可对话。渠道跟随平台协作通知设置，当前使用{activeKey === 'FEISHU' ? '飞书' : '钉钉'}。
          </Text>
        </Space>
      </Card>
      <Card>
