-- Exact pre-V037 canonical schema fixture.
-- Source: origin/master docs/autowonder-schema.sql @ 66d7934587824007ca3106672bc1fb13519eff77
-- Reconciled with the already-shipped pre-V037 V017 delivery-reply migration, which the
-- canonical aggregate file had not folded in. No V037 object is present.
-- This fixture intentionally contains the complete production schema immediately before V037.
-- =============================================================================
-- AutoWonder 全库 DDL（MySQL 8 / InnoDB / utf8mb4）
-- 生成依据：PRD + 详设 01-08（docs/superpowers/specs/）
-- 说明：
...
CREATE TABLE IF NOT EXISTS `platform_im_channel_config` (
  ...
  `credential_ref` VARCHAR(1024)   DEFAULT NULL COMMENT '加密后的 application secret 引用',
...
CREATE TABLE IF NOT EXISTS `executor` (
  ...
  `name`           VARCHAR(128)    NOT NULL,
  `token_ref`      VARCHAR(256)    NULL     COMMENT 'KeyCenter 引用（WS 鉴权 token）',
...
CREATE TABLE IF NOT EXISTS `system_setting` (
  ...
  `credential_ref` VARCHAR(256)    DEFAULT NULL COMMENT 'is_secret 时的 KeyCenter 引用',
  `gmt_create`     DATETIME(3)     NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `gmt_modified`   DATETIME(3)     NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  `creator_id`     BIGINT UNSIGNED DEFAULT NULL,
  `modifier_id`    BIGINT UNSIGNED DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `dingtalk_robot_binding` (
  ...
  `credential_ref` VARCHAR(512) NOT NULL COMMENT 'appSecret 密文引用(KeyCenter)',
