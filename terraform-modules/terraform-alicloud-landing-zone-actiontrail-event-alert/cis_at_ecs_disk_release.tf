resource "alicloud_log_alert" "cis_at_ecs_disk_release" {
  ...
  alert_displayname = "ECS Cloud Disk Released Alert"
  ...
  annotations {
    key   = "desc"
    value = "账号$${account_id}下的ECS云盘$${disk_id}（区域：$${region_id}）已被释放，请检查是否存在风险。操作账号ID：$${ram_user_id},账号名：$${user_name},账号类型:$${user_type}。"
  }
  annotations {
    key   = "title"
    value = "ECS云盘释放告警"
  ...
  query_list {
    ...
    query          = "event.serviceName: Ecs and event.eventName: DeleteDisk and event.eventRW: Write and event.resourceType : \"ACS::ECS::Disk\" | SELECT COALESCE(account_id, recipientAccountId) as account_id, ram_user_id, resourceArray[num] as disk_id, arbitrary(user_type) as user_type, arbitrary(user_name) as user_name, arbitrary(region_id) as region_id FROM (SELECT \"event.userIdentity.accountId\" as account_id, \"event.recipientAccountId\" as recipientAccountId, \"event.userIdentity.principalId\" as ram_user_id, split(\"event.resourceName\", ';') as resourceArray, array_position(split(\"event.resourceType\", ';'), 'ACS::ECS::Disk') as num, case when \"event.userIdentity.type\"='root-account' then '阿里云主账号' when \"event.userIdentity.type\"='ram-user' then 'RAM用户' when \"event.userIdentity.type\"='assumed-role' then 'RAM角色' when \"event.userIdentity.type\"='system' then '阿里云服务' else \"event.userIdentity.type\" end as user_type, \"event.userIdentity.userName\" as user_name, \"event.acsRegion\" as region_id FROM log) where num > 0 group by account_id, ram_user_id, disk_id limit 10000"
