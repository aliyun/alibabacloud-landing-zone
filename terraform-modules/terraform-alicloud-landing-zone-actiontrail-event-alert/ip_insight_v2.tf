resource "alicloud_log_alert" "ip_insight_v2" {
  ...
  alert_displayname = "IpInsight Alert(New Version)"
  ...
  query_list {
    ...
    query          = "event.insightDetails.insightType: IpInsight | select array_agg(distinct \"event.acsRegion\") as regions, array_agg(distinct \"event.insightDetails.insightObject\") as ips, count(1) as cnt from log"
