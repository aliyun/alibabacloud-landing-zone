resource "alicloud_log_alert" "cis_at_cfw_loose_block" {
  ...
  alert_displayname = "Alert of Cloudfirewall Switched to Loose Interception Mode"
  ...
  annotations {
    key   = "desc"
    value = "账号$${account_id}下的云防火墙的威胁引擎被切换为宽松拦截模式。操作账号ID：$${ram_user_id},账号名：$${user_name},账号类型:$${user_type}。"
  }
  annotations {
    key   = "title"
    value = "云防火墙威胁引擎切换至宽松拦截模式告警"
  }
