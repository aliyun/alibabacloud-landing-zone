resource "alicloud_log_alert" "cis_at_cfw_ai_off" {
  ...
  alert_displayname = "Alert of Turning off of Cloudfirewall Intelligent Defense"
  ...
  annotations {
    key   = "desc"
    value = "账号$${account_id}下的云防火墙的智能防御被关闭。操作账号ID：$${ram_user_id},账号名：$${user_name},账号类型:$${user_type}。"
  }
  annotations {
    key   = "title"
    value = "云防火墙智能防御关闭告警"
